...
Beyond Renovate (already planned), should gdev include dependency freshness checking (`gdev outdated`) and coordinated update workflows?
...
## Analysis: Should gdev Include `gdev outdated`?
...
2. **Consulting context**: When picking up a stale client project, `gdev outdated` immediately shows the maintenance debt.
...
## Coordinated Updates: `gdev update`
...
```
$ gdev update
[1/4] Checking for gdev updates... v1.2.0 -> v1.3.0 available
[2/4] Regenerating configs for v1.3.0... 3 files updated
[3/4] Updating devenv inputs... 2 inputs updated
[4/4] Application dependencies... skipped (use gdev update --deps or Renovate)
```
...
**Recommendation: Include `gdev update` for steps 1-3 only.** This is the "keep gdev infrastructure current" command. Application dependency updates remain Renovate's domain.
...
|---------|---------------|-----------|
| `gdev outdated` (thin wrapper) | **Include** | One command for all ecosystems, low complexity |
| `gdev outdated` (unified analysis) | **Exclude** | Too complex, Renovate does this better |
| `gdev update` (self + configs + devenv) | **Include** | Coordinated infrastructure update, safe |
| `gdev update --deps` (app deps) | **Exclude** | Renovate's domain, dangerous for unattended |
...
- [x] Concrete examples -- per-ecosystem command table, gdev update output mockup, age-gating config names
