...
//
// It emits only Claude Code files: the files of non-agent tools (see
...
func (g *ClaudeCodeGenerator) Generate(answers types.WizardAnswers) ([]types.GeneratedFile, error) {
	...
	// 4b. AlwaysOn agent-tool files (Standard+). Only agent tools belong to
	// this generator: every other tool's files (gitleaks, semgrep, changelog,
	// ...) are project files that devinit's tool-files producer generates
	// whether or not Claude Code is configured.
	reg := toolreg.DefaultRegistry()
	for _, tool := range reg.All() {
		if tool.Default != toolreg.AlwaysOn || !tool.IsAgentTool() {
			continue
		}
	...
	// 15. Explicitly enabled (opt-in) agent tools. Non-agent tools are
	// generated by devinit's tool-files producer, not here.
