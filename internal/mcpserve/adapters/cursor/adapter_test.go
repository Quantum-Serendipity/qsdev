...
func TestID(t *testing.T) {
	t.Parallel()
	if id := New().ID(); id != aiframework.Cursor {
		t.Errorf("ID() = %q, want %q", id, aiframework.Cursor)
	}
}
...
func TestRegisteredInDefaultRegistry(t *testing.T) {
	...
	for _, a := range spi.DefaultRegistry().All() {
		if a.ID() == aiframework.Cursor {
			found = true
		}
	}
	if !found {
		t.Fatal("cursor adapter not present in spi.DefaultRegistry()")
	}
}
...
func TestApplies(t *testing.T) {
	...
	present := t.TempDir()
	if err := os.MkdirAll(filepath.Join(present, ".cursor", "rules"), 0o755); err != nil {
		t.Fatalf("creating .cursor/rules: %v", err)
	}
	if !a.Applies(context.Background(), present) {
		t.Error("Applies(.cursor/rules present) = false, want true")
	}
}
...
func TestToolsSurface(t *testing.T) {
	...
	for _, reg := range tools {
		if !strings.HasPrefix(reg.Name, "qsdev_cursor_") {
			t.Errorf("tool %q lacks qsdev_cursor_ prefix", reg.Name)
		}
...
func TestCapabilitiesTool(t *testing.T) {
	...
	if caps["tool_ceiling"].(int) != 40 {
		t.Errorf("tool_ceiling = %v, want 40", caps["tool_ceiling"])
	}
	if caps["tool_ceiling_kind"].(string) != "client-side" {
		t.Errorf("tool_ceiling_kind = %v, want client-side", caps["tool_ceiling_kind"])
	}
	if caps["config_format"].(string) != "MDC" {
		t.Errorf("config_format = %v, want MDC", caps["config_format"])
	}
	if caps["enforcement_tier"].(string) != "advisory" {
		t.Errorf("enforcement_tier = %v, want advisory", caps["enforcement_tier"])
	}
	if caps["native_hooks"].(bool) {
		t.Error("native_hooks = true, want false")
	}
}
...
func TestConfigToolGated(t *testing.T) {
	...
	if payload["framework"] != string(aiframework.Cursor) {
		t.Errorf("framework = %v, want cursor", payload["framework"])
	}
...
func TestInfoTool(t *testing.T) {
	...
	if structured["framework"].(string) != string(aiframework.Cursor) {
		t.Errorf("framework = %v, want cursor", structured["framework"])
	}
	if structured["project_root"].(string) != root {
		t.Errorf("project_root = %v, want %v", structured["project_root"], root)
	}
	if structured["config_convention"].(string) != configConvention {
		t.Errorf("config_convention = %v, want %v", structured["config_convention"], configConvention)
	}
	langs := structured["languages"].([]string)
	foundGo := false
	for _, l := range langs {
		if l == "go" {
			foundGo = true
	...
	if !foundGo {
		t.Errorf("languages = %v, want to include go", langs)
	}
