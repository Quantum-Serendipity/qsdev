...
func TestID(t *testing.T) {
	t.Parallel()
	if id := New().ID(); id != aiframework.ContinueDev {
		t.Errorf("ID() = %q, want %q", id, aiframework.ContinueDev)
	}
}
...
func TestRegisteredInDefaultRegistry(t *testing.T) {
	...
	for _, a := range spi.DefaultRegistry().All() {
		if a.ID() == aiframework.ContinueDev {
			found = true
		}
	}
	if !found {
		t.Fatal("cline adapter not present in spi.DefaultRegistry()")
	}
}
...
func TestApplies(t *testing.T) {
	...
	present := t.TempDir()
	if err := os.MkdirAll(filepath.Join(present, ".continue"), 0o755); err != nil {
		t.Fatalf("creating .continue: %v", err)
	}
	if !a.Applies(context.Background(), present) {
		t.Error("Applies(.continue present) = false, want true")
	}
}
...
func TestToolsSurface(t *testing.T) {
	...
	for _, reg := range tools {
		if !strings.HasPrefix(reg.Name, "qsdev_cline_") {
			t.Errorf("tool %q lacks qsdev_cline_ prefix", reg.Name)
		}
...
func TestCapabilitiesTool(t *testing.T) {
	...
	if caps["config_format"].(string) != "YAML" {
		t.Errorf("config_format = %v, want YAML", caps["config_format"])
	}
	if caps["family"].(string) != "Continue.dev" {
		t.Errorf("family = %v, want Continue.dev", caps["family"])
	}
...
func TestConfigToolGated(t *testing.T) {
	...
	if payload["framework"] != string(aiframework.ContinueDev) {
		t.Errorf("framework = %v, want continue", payload["framework"])
	}
...
func TestInfoTool(t *testing.T) {
	...
	if structured["framework"].(string) != string(aiframework.ContinueDev) {
		t.Errorf("framework = %v, want continue", structured["framework"])
	}
