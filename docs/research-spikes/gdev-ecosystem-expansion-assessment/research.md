...
- ~~Which cloud providers/CLI tools should qsdev configure out of the box vs leave to profiles?~~ Answered: See cloud-k8s-tooling-research.md (4-tier framework)
...
- ~~Should client profiles live in `.qsdev.local.yaml` or a separate config?~~ Answered: `~/.qsdev/clients/<name>.yaml`, sops+age encrypted. See client-profile-design.md
...
5. **Shell/workstation configuration** — Rather than bundling modern coreutils (ripgrep, fd, bat, fzf, jq, yq, delta, eza, zoxide) into devenv, explore `qsdev setup` providing a personal shell configuration mode that manages `~/.qsdev/shell/` dotfile fragments. These tools belong in the engineer's personal shell, not per-project devenv.
...
10. **Copier template integration** — `qsdev init --from <template>` for firm-wide project standards.

### Lower-priority but worthwhile

11. **.editorconfig generation** — Always-generate, universally safe.
12. **Observability sidecar** — `qsdev enable observability` via Docker grafana/otel-lgtm.
...
15. **.vscode/extensions.json** — Opt-in via `qsdev enable vscode`.
