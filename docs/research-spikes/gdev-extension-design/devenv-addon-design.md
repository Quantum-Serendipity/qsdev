# gdev devenv Addon — Design Specification
...
The addon registers a single config key `devenv` that persists the full environment configuration. This enables `gdev devenv update` to regenerate files without re-running the wizard.
...
Everything the wizard collects is saved here. This is the source of truth for `gdev devenv update`.
...
**Saved to gdev config** (survives across sessions):
...
The addon registers five bootstrap steps that execute during `gdev bootstrap`. Each step uses charmbracelet/huh forms for interactive input and supports headless mode via CLI flags.
...
```go
const envrcContent = `# Auto-generated by gdev devenv addon
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if fileExists("devenv.nix") && !flagForce {
                ...
                huh.NewConfirm().
                    ...
                    Value(&overwrite).Run()
                if !overwrite {
                    return fmt.Errorf("cancelled — use 'gdev devenv update' to regenerate from saved config")
                }
            }
...
Re-generates files from saved config without running the wizard. Useful after manually editing the gdev config YAML, or when the addon's templates have been updated.
    ...
    return &cobra.Command{
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if persisted.IsDefault() {
                return fmt.Errorf("no saved devenv configuration — run 'gdev devenv init' first")
            }
...
gdev devenv init --lang=go --go-version=1.24 --services=postgres --pg-db=myapp_dev --yes

# Re-use saved config from previous run
gdev devenv init --yes

# CI: generate from explicit flags, overwrite existing
gdev devenv init --lang=go,typescript --services=postgres,redis --force --yes
```
...
```bash
# Auto-generated by gdev devenv addon
# Run 'direnv allow' to activate
...
- **bootstrap addon**: The devenv addon registers its steps with `bootstrap.WithSteps()`. If bootstrap is not configured, the steps are simply not run during `gdev bootstrap`, but the `gdev devenv init` command still works standalone.
...
3. **Container generation**: A `gdev devenv containerize` command could add OCI container configuration to an existing `devenv.nix`. This is a natural extension of the `add-service` pattern.

4. **SecretSpec integration**: For teams using secrets management, a `gdev devenv add-secrets` command could configure the secretspec block in `devenv.yaml` and create the `secretspec.toml` manifest.

5. **Template updates (Copier-style)**: When the addon's templates evolve, `gdev devenv update` regenerates from saved config. This is a simple but effective update mechanism. A more sophisticated approach would diff the generated output against the current file and offer a merge, but the regenerate-from-config approach is sufficient for v1.

6. **Profile support**: devenv.sh supports profiles for environment variants. The addon could grow `gdev devenv add-profile <name>` to manage these.
