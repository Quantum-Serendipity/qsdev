...
These units amend Phase 6 (Wizard & Orchestration) of the gdev-secure-devenv-bootstrap implementation plan. They add Copier-based project scaffolding support to `gdev init`, enabling firm-wide project templates with lifecycle update capability.
...
**Context:** Templates are git repos containing a `copier.yaml` questionnaire. Engineers should not need to remember full URLs. The registry lives in `~/.qsdev/templates.yaml` (user-level) and can also be supplied by organization config (`.gdev.yaml` in a shared config repo, propagated via the three-layer config from Phase 13). Resolution order: exact URL/path passthrough → user registry lookup → org registry lookup → error.

**Desired Outcome:** `gdev init --from ts-api` resolves `ts-api` to a full git URL. `gdev init --from https://github.com/highspring/template-ts-api.git` passes through unchanged. `gdev init --from ./local-template` resolves to an absolute path.
...
4. Support org-level registry via `.gdev.yaml` `templates:` key (merged with user registry, user overrides org for name collisions).
5. Implement `gdev template list` command showing name, URL, and source (user/org) for each registered template.
6. Implement `gdev template add <name> <url>` to register a template in the user registry.
7. Implement `gdev template remove <name>` to unregister a template from the user registry.
...
- [ ] `gdev template list` shows all templates with source labels
- [ ] `gdev template add/remove` modifies `~/.qsdev/templates.yaml`
- [ ] Registry file is created on first `gdev template add` if absent
...
3. Implement `Copy(templateURL, destDir string, opts CopyOpts) error`:
   ...
   - `--data key=value` for pre-answered questions from gdev config
...
- `implementation-plans/gdev-secure-devenv-bootstrap/plan.md § Design Principles` — "Curate, don't reinvent"

**Status:** Not Started

---

### Unit 5.9: `gdev init --from` Orchestration Flow

**Description:** Wire the `--from` flag into `gdev init`, orchestrating the Copier-first-then-gdev-init pipeline: resolve template, run Copier questionnaire, then run normal gdev init on the generated project.

**Context:** When `--from` is specified, gdev init gains a pre-phase: template scaffolding. The flow is (1) resolve template name, (2) gate on Copier availability, (3) run `copier copy` to generate project files, (4) run the normal gdev init pipeline (detection, wizard, generation) on the now-populated directory. The Copier questionnaire and gdev wizard are sequential — Copier asks template-specific questions first, then gdev asks environment configuration questions. The generated project may include a `.gdev.yaml` with org defaults, which the gdev wizard consumes as pre-populated answers.

**Desired Outcome:** `gdev init --from ts-api` produces a project with both Copier-templated files (README, CI workflows, security policies, CLAUDE.md) AND gdev-generated config (devenv.nix, devenv.yaml, settings.json).

**Steps:**
1. Add `--from <template>` flag to `gdev init` command (Unit 5.1 extended).
2. When `--from` is set, insert pre-phase into init orchestration:
   ...
   d. If the template generated a `.gdev.yaml`, load it as project config (feeds into wizard defaults).
...
**Acceptance Criteria:**
- [ ] `gdev init --from ts-api` resolves template and runs Copier
...
- [ ] gdev wizard pre-populates from Copier-generated `.gdev.yaml` if present
...
- [ ] `gdev init --from ts-api --yes` runs both Copier and gdev non-interactively
- [ ] `gdev init` without `--from` is unchanged (no regression)
...
- `implementation-plans/gdev-secure-devenv-bootstrap/phases/06-wizard-orchestration.md § Unit 5.1` — existing init orchestration flow
...
### Unit 5.10: `gdev update --template` Flow

**Description:** Implement the `gdev update --template` command that runs `copier update` to pull latest template changes into an existing Copier-templated project, then re-runs gdev init in update mode to reconcile gdev-generated files.

**Context:** This is the critical consulting lifecycle feature. When the firm updates its base template (new CI pipeline stages, updated security policies, revised CLAUDE.md conventions), engineers run `gdev update --template` to pull those changes forward. Copier's 3-way merge applies template changes while preserving local modifications. After Copier updates the scaffolding, gdev re-runs its own update flow (Phase 8, Unit 6.1) to reconcile any changes to `.gdev.yaml` or other gdev-managed files that the template update may have affected.

**Desired Outcome:** `gdev update --template` brings an existing project forward to the latest template version, then reconciles gdev config, with clear reporting of what changed and any merge conflicts.

**Steps:**
1. Add `--template` flag to `gdev update` command.
2. Detect whether the project is Copier-templated by checking for `.copier-answers.yml`.
3. If not Copier-templated, return clear error: "This project was not created from a template. Use `gdev init --from <template>` to start from a template."
...
7. If `.gdev.yaml` was modified by the template update, re-run gdev init in update mode (Phase 8, Unit 6.1) to reconcile devenv.nix, settings.json, CLAUDE.md, etc.
8. If `.gdev.yaml` was not modified, skip gdev re-init but report that gdev config is unchanged.
...
**Acceptance Criteria:**
- [ ] `gdev update --template` runs `copier update` on a Copier-templated project
...
- [ ] Re-runs gdev init in update mode when `.gdev.yaml` changed
- [ ] Skips gdev re-init when `.gdev.yaml` unchanged
- [ ] `gdev update --template --yes` runs non-interactively
...
- `implementation-plans/gdev-secure-devenv-bootstrap/phases/08-migration-update-polish.md § Unit 6.1` — gdev update flow with hash-based modification detection
...
**Desired Outcome:** `gdev init --from ts-api --yes` and `gdev init --from ts-api --data project_name=acme --data author=colin --yes` both produce a complete project with zero interactive prompts.

**Steps:**
1. Map `gdev init --yes` to `copier copy --defaults` (use template-defined defaults for all Copier questions).
2. Implement `--data key=value` flag (repeatable) on `gdev init --from` to pass answers to Copier.
...
5. Map `gdev update --template --yes` to `copier update --defaults`.
6. Document the full non-interactive invocation in `gdev init --help`.
7. Validate with a test scenario: CI script that creates a project from template, runs gdev init, and verifies all expected files exist.

**Acceptance Criteria:**
- [ ] `gdev init --from ts-api --yes` uses Copier defaults, no prompts
- [ ] `gdev init --from ts-api --data project_name=acme --yes` overrides specific Copier answers
...
- [ ] `gdev update --template --yes` runs non-interactively
- [ ] All flags documented in `gdev init --help` and `gdev update --help`

**Research Citations:**
- `implementation-plans/gdev-secure-devenv-bootstrap/phases/06-wizard-orchestration.md § Unit 5.5` — existing non-interactive/CI flag mapping
...
**Description:** Define and document the conventions for firm-maintained Copier templates that integrate with gdev, including what files a template should contain, how `.gdev.yaml` in templates pre-configures gdev, and how template-provided CLAUDE.md interacts with gdev-generated CLAUDE.md.

**Context:** The value of Copier integration depends on well-structured templates. This unit does not implement code — it produces the specification that template authors follow. The spec covers file layout conventions, `.gdev.yaml` integration points, CLAUDE.md section conventions, and CI pipeline templates. This is embedded documentation generated by `gdev init --from` when creating a new template repo.
...
1. Define recommended template directory structure:
   ...
   │   ├── .gdev.yaml        # Pre-configured gdev org defaults
   ...
   │   └── ...
   ```
2. Document `.gdev.yaml` integration: templates can set `compliance_level`, `security_profile`, `default_services`, `org_registry`, and other gdev config that the wizard consumes as defaults.
...
4. Document CI pipeline template conventions: GitHub Actions workflows that reference gdev commands (`gdev check`, `gdev status --json`).
...
6. Embed this spec as a help topic: `gdev template --help-authoring` or as generated documentation in a new template scaffold.

**Acceptance Criteria:**
- [ ] Template directory structure documented with rationale for each convention
- [ ] `.gdev.yaml` integration points documented (which keys templates should set)
...
- [ ] Spec is accessible via `gdev template --help-authoring` or similar
...
- `research-spikes/gdev-team-config-onboarding/research.md § Three-Layer Config` — `.gdev.yaml` as org config carrier
- `implementation-plans/gdev-secure-devenv-bootstrap/phases/04-claude-code-addon-core-generation.md` — CLAUDE.md section marker pattern
...
- [ ] `gdev init --from <template-url>` creates a project from a Copier template and runs gdev init on the result
- [ ] `gdev init --from <short-name> --yes` resolves name from registry and runs fully non-interactive
- [ ] `gdev update --template` pulls latest template changes and reconciles gdev config
- [ ] `gdev template list/add/remove` manages the template registry
...
- **Units 5.7-5.12 depend on Unit 5.1** (command hierarchy — `--from` flag added to `gdev init`).
- **Unit 5.9 depends on Units 5.7 and 5.8** (needs registry resolution and Copier invocation).
- **Unit 5.10 depends on Unit 5.8** (needs Copier invocation) and **Phase 8, Unit 6.1** (gdev update flow).
