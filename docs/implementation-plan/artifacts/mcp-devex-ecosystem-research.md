...
| **Language** | Python |
| **Version** | 1.28.0 |
| **Consulting relevance** | **Reference architecture, not a direct dependency.** gdev's `qsdev devenv doctor` command should implement similar patterns: repo-specific checks, guided fixes, diagnostic output. The check/fix pattern is proven at Sentry's scale. |
...
```
.mcp.json (generated by qsdev claudecode addon)
├── Always-on servers
