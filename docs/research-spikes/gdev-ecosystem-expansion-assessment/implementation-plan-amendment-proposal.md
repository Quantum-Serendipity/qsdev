...
| 5.9 | `qsdev init --from` Orchestration Flow | Copier |
| 5.10 | `qsdev update --template` Flow | Copier |
...
**Key design decisions**:
- `--from <template>` runs Copier first, then normal qsdev init on generated project
...
**Current scope**: `qsdev init --update`, three-way merge, section markers, team standards versioning
...
**Key design decisions**:
- `.editorconfig` is always generated by `qsdev init` (universally safe, editor-agnostic)
- `.vscode/extensions.json` is opt-in via `qsdev enable vscode`
...
**Key design decisions**:
- `qsdev setup --shell` manages `~/.qsdev/shell/` (personal, not per-project)
...
**Key design decisions**:
- `qsdev mcp list` shows tool counts and budget remaining
...
- `qsdev enable observability` / `qsdev disable observability` lifecycle
...
**Current scope**: `.qsdev.yaml` project config, three-layer resolution, four onboarding modes, `qsdev check` CI enforcement
...
| 13.10 | Baked Config Propagation to .qsdev.yaml |
| 13.11 | Profile-Aware Compliance Enforcement in qsdev check |

**Key design decisions**:
- `ClientProfile` Go struct versioned independently of `.qsdev.yaml`
- Two-layer security: sops+age at rest, SecretSpec at runtime
- Non-secret values baked into `.qsdev.yaml` `client` block
...
| 10 | Distribution & Self-Bootstrapping | Add: "+ `qsdev setup --shell` personal workstation configuration" |
...
> **Minimum devenv version: 2.0** (released March 2026). The plan depends on devenv 2.0 features: DAG-based task system (parallel execution, caching, JSON data passing, process integration), `devcontainer.enable` native toggle, and namespace task convention. `qsdev doctor` must check devenv version and warn if < 2.0.
...
| Atlassian MCP | MCP server (optional) | .mcp.json via `qsdev enable mcp-atlassian` |
| Linear MCP | MCP server (optional) | .mcp.json via `qsdev enable mcp-linear` |
| Slack MCP | MCP server (optional) | .mcp.json via `qsdev enable mcp-slack` |
| Datadog MCP | MCP server (optional) | .mcp.json via `qsdev enable mcp-datadog` |
| Grafana MCP | MCP server (optional) | .mcp.json via `qsdev enable mcp-grafana` |
...
- **Phase 18** (Cross-Platform): Cloud CLI installation across OS families, `qsdev setup --shell` validation
...
- **Phase 22** (Agentic/Compliance/DX): Client profile wizard flows, profile compliance enforcement, `qsdev setup --shell` verification
