...
2. **Config schema version** -- the `.gdev.yaml` format version (e.g., 1, 2, 3)
...
**`version`** (integer): Format version of the `.gdev.yaml` file itself. Bumped when the schema changes in a backward-incompatible way. gdev reads this, loads the appropriate schema parser, and migrates to the latest internal representation.
...
- Migrations run in-memory only -- the file on disk is not rewritten unless the user explicitly runs `gdev config migrate`
...
**Problem:** Engineer A runs gdev v0.16.0 which generates updated pre-commit hooks. Engineer B has gdev v0.15.0 and runs `gdev init --update`, which would downgrade the hooks.
...
1. **`.gdev.yaml` `gdev_version` sets the floor.** All team members must meet this minimum.
...
3. **Newer versions can update; older versions refuse to downgrade.** This creates a "ratchet" effect -- once anyone on the team generates with v0.16.0, the `.gdev.yaml` should be updated to `gdev_version: ">= 0.16.0"`.

4. **`gdev init --update` with `--bump-version` flag** updates the `gdev_version` constraint to match the current binary, signaling to the team that they need to update.
...
Migration function transforms the data. The file on disk retains `version: 1` until the user runs `gdev config migrate`.
...
1. **`.gdev.yaml` manually edited with syntax error:** gdev must report the YAML parse error with line number and suggest checking recent git changes. Never proceed with a partially-parsed config.

2. **Config version newer than binary understands:** Error with "This project's .gdev.yaml uses config version 4, but your gdev only supports up to version 3. Run `gdev self-update`."
...
4. **Multiple engineers bump version simultaneously:** Git merge conflict on the `gdev_version` field in `.gdev.yaml`. This is a feature, not a bug -- it forces the team to agree on the version floor.

5. **Rollback scenario:** Team discovers a gdev version has a bug and needs to roll back. The `gdev_version` constraint is lowered in `.gdev.yaml`, but generated files from the buggy version remain. `gdev init --update --force` regenerates everything from the rolled-back version.
