---
name: qsdev-disable
description: Disable a tool from the qsdev project configuration.
...
4. **Disable the tool**: Run `qsdev disable $tool` where `$tool` is the tool name provided or selected.

5. **Verify**: Run `qsdev status --json` to confirm the tool is now disabled.
