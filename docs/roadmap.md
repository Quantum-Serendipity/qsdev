...
- **AI agent tooling** (P23) — Agent-postmortem and version-sentinel MCP tool modules, served by the universal MCP server (`qsdev mcp serve --module <name>`) with catalog-driven auto-injection; `qsdev mcp status` and `qsdev mcp list` diagnostic commands
