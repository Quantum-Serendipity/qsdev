...
// launched process when the timeout fires. Like the unix version it runs with
// the server's environment minus its credentials (childEnv). Child processes
// are not guaranteed to be reaped on these platforms, which is acceptable
// because nix_run targets unix-like development hosts. The process runs in dir
// (the server's working directory when dir is empty).
