...
| `research-spikes/gdev-team-config-onboarding/` | Three-layer config pattern, four onboarding modes, Terraform-style version constraints, `gdev check` CI enforcement, client profiles with compliance levels, consulting lifecycle (teardown/archive/evidence) |
| `research-spikes/gdev-dx-polish/` | gdev repair/info/outdated/update/teardown design, git workflow automation, shell integration, task runner rejection (devenv 2.0 native), 10 rejected features with rationale |
| `research-spikes/effective-readme-formats/research.md` | 12 winning README patterns, 11 anti-patterns, effectiveness spectrum across 20 developer tools, conversion funnel model, actionable template, documentation UX research |
| `research-spikes/go-sbom-goreleaser-pipeline/research.md` | Syft SBOM generation (SPDX 2.3 + CycloneDX 1.5), cosign keyless signing via OIDC (SLSA Build Level 2), govulncheck OpenVEX (97.5% false-positive suppression), Trivy exclusion (March 2026 compromise), distribution channel integrity |
| `research-spikes/security-tooling-evaluation-gdev/research.md` | 5-tool evaluation (Prempti, npm-scan, reasoning-core, Cloudberry, Sense), 23 borrowable design patterns for native Go implementation, optional integration points, no tool recommended as default |
| `research-spikes/gdev-agent-self-protection-design/research.md` | 32-rule enforcement layer, exit code 2 enforcement (not JSON ask), path canonicalization, fail-closed harness, three-tier bypass policy, per-rule monitor mode, Go binary consolidation |
...
9. **Single binary, zero prerequisites.** gdev ships as a static Go binary (`CGO_ENABLED=0`) with all templates, skills, and completions embedded via `embed.FS`. Installation requires only curl (Unix) or PowerShell (Windows). The binary self-bootstraps everything else via `gdev setup`.
...
11. **Detect, don't assume.** Before installing or configuring anything, detect what's already present. `gdev doctor` reports system state; `gdev setup` acts only on gaps. Never overwrite user-installed tools or assume a specific environment.
...
13. **Every tool is individually toggleable.** `gdev enable <tool>` adds it; `gdev disable <tool>` cleanly removes it — including shared-file sections in devenv.nix, settings.json, CLAUDE.md, and .mcp.json. Tool adoption is a reversible, low-risk decision that encourages experimentation. File ownership tracking ensures no orphaned artifacts.
...
| 9 | Cross-Platform System Detection | Not Started | Phase 1 | OS detection engine (12 distro families), package manager abstraction (12 managers), tool prerequisite detection, shell integration, privilege escalation, `gdev doctor`, `gdev setup` |
| 10 | Distribution & Self-Bootstrapping | Not Started | Phases 1, 9 | GoReleaser multi-platform builds, install scripts (bash + PowerShell), Homebrew/Scoop/APT/RPM packaging, self-update, shell completions, `gdev version` |
...
| 12 | Extended Integrations & Lifecycle | Not Started | Phases 1-8, 9, 11 | Tool lifecycle management (`gdev enable/disable/status/list`), Semgrep (SAST), Gitleaks (secrets), Grype+Syft+Cosign (containers), ScanCode (licenses), SecretSpec (dev secrets), CI workflow generation, Context7 MCP, git-cliff (changelog), retroactive lifecycle for Phase 4/11 tools |
| 13 | Project Configuration & Team Standards | Not Started | Phases 1, 6, 8, 12 | `.gdev.yaml` project config file, three-layer resolution (binary → project → local), four onboarding modes (Create/Join/Update/Repair), `gdev check` CI enforcement, config versioning, client-specific profiles with compliance levels |
...
| 15 | Health, Status & Compliance Reporting | Not Started | Phases 1, 12, 13, 14 | `gdev status` with scoring (0-100, A-F, conformance labels), 6-category drift detection, `gdev evidence` compliance reports, machine-readable output (JSON/SARIF), badge generation, team aggregation pipeline |
| 16 | Developer Experience Polish | Not Started | Phases 3, 9, 10, 12, 13, 15 | `gdev repair` self-healing, `gdev info`, `gdev outdated`, `gdev update` coordinated updates, `gdev teardown` clean exit, git workflow automation, shell & environment integration |
| 16.2 | Developer Experience Polish — README | Not Started | Phases 10, 16 | Research-backed README as conversion funnel: 12 winning patterns, concrete comparison, 30-second quick-start, terminal recordings, honest positioning, platform-aware installation |
| 16.3 | Developer Experience Polish — Pitch & Adoption | Not Started | Phases 10, 15, 16, 16.2 | Elevator pitch scripts (4 audiences x 3 lengths), 15-minute demo package with fallbacks, deep-dive presentation slides (35 slides), leave-behind materials (5-component package), adoption playbook (20-week timeline), Show HN launch post, demo environment |
...
| 18 | Cross-Platform Installation Validation | Not Started | Phases 17, 9, 10 | Install script E2E on 24 OS targets, `gdev doctor` validation across all distro families, `gdev setup --dry-run` verification, package manager distribution testing (Homebrew/Scoop/APT/RPM), self-update + shell completions |
| 19 | Ecosystem & Configuration Onboarding Validation | Not Started | Phases 17, 1-8, 6, 13 | Greenfield `gdev init` for all 27 ecosystems, brownfield onboarding against 14+ real open-source repos, polyglot project composition, detection accuracy, `.gdev.yaml` Join mode testing |
...
24. **No tool lifecycle management exists.** `gdev init` is a one-way door — there's no `gdev disable <tool>` or `gdev enable <tool>`. Developers can't experiment with tools without committing to manual cleanup. This is the highest-priority gap for Phase 12.
...
43. **Three-layer config with security floor.** Binary defaults → `.gdev.yaml` (project, checked into git) → `.gdev.local.yaml` (developer, gitignored). Security level acts as a floor — local overrides cannot weaken project security policy. Pattern validated by EditorConfig, ESLint flat config, Renovate shared presets, and mise's `.mise.local.toml`.
44. **Four onboarding modes eliminate guesswork.** `gdev init` auto-detects mode: Create (fresh project), Join (existing .gdev.yaml, new developer), Update (newer gdev available), Repair (config drift detected). Join mode targets under 2 minutes from git clone to productive environment.
...
46. **Dynamic context injection is the key pattern.** The `!`gdev doctor --json`` preprocessor in skill files runs shell commands before Claude sees the content, injecting live system state. This makes gdev skills context-aware without stale documentation.
...
51. **Conservative repair, never destructive.** `gdev repair` backs up before any change, never auto-modifies user-edited devenv.nix (generates .new + diff instead), and uses `--dry-run` preview by default. Recovery from 4 failure categories: Nix/devenv, config corruption, tool failures, environment drift.
52. **Ten features explicitly rejected.** Task runner (devenv native), container management (Docker/Podman exist), CI execution, deployment, code scaffolding, IDE config beyond Claude Code, OTEL infrastructure (just env vars), package manager installation (gdev setup), Git server API, vulnerability database. Three-test framework: purpose-built tool exists? file generation or runtime? compounds with existing features?

## Current Status
No implementation work has started. Phase 1 is the entry point. The MVP plan encompasses 24 phases: 17 development phases (1-16 plus 16.2 and 16.3) and 6 validation phases (17-22). Post-MVP enhancements add phases 23-43 (see `additional-enhancements-plan.md`).
