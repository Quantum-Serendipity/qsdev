...
// Behavior varies based on RenderOptions: quiet, verbose, and fix modes.
func RenderText(report *PostureReport, w io.Writer, opts RenderOptions) error {
	if opts.Fix {
...
func renderQuiet(report *PostureReport, w io.Writer) error {
...
// remediation hints.
func renderVerbose(report *PostureReport, w io.Writer, opts RenderOptions) error {
	pass, partial, skip, fail := Indicators(opts.UseColor)
...
func renderFix(report *PostureReport, w io.Writer) error {
...
func renderConformanceLevel(w io.Writer, name string, level ConformanceLevel, pass, fail string) {
	ind := pass
	verdict := "PASS"
	if !level.Pass {
		ind = fail
...
func indicatorForLayer(status LayerStatus, pass, partial, skip, fail string) string {
	switch status {
	case LayerEnabled:
		return pass
	case LayerPartial:
...
func driftIndicator(severity DriftSeverity, _, _, _, fail string) string {
	switch severity {
	case DriftCritical, DriftError:
		return fail
	case DriftWarning:
