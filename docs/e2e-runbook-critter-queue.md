...
- `context7` — library documentation lookups (npx @upstash/context7-mcp@4.1.1)
- `github` — repository/issue management (github-mcp-server stdio, Nix package)
- `socket` — supply chain security analysis (HTTP https://mcp.socket.dev/)
- `semble` — semantic code search (uvx --from semble[mcp]==0.6.0 semble)
...
```
You should see: MCP server entry with `uvx --from semble[mcp]==0.6.0 semble` command, and the semble-search agent file. Both were generated at init since semble is AlwaysOn.
