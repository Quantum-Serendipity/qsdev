...
func TestID(t *testing.T) {
	t.Parallel()
	if id := New().ID(); id != aiframework.Windsurf {
		t.Errorf("ID() = %q, want %q", id, aiframework.Windsurf)
	}
}
...
func TestRegisteredInDefaultRegistry(t *testing.T) {
	...
	for _, a := range spi.DefaultRegistry().All() {
		if a.ID() == aiframework.Windsurf {
			found = true
		}
	}
	if !found {
		t.Fatal("windsurf adapter not present in spi.DefaultRegistry()")
	}
}
...
func TestApplies(t *testing.T) {
	...
	present := t.TempDir()
	if err := os.WriteFile(filepath.Join(present, ".windsurfrules"), []byte("# rules\n"), 0o644); err != nil {
		t.Fatalf("writing .windsurfrules: %v", err)
	}
	if !a.Applies(context.Background(), present) {
		t.Error("Applies(.windsurfrules present) = false, want true")
	}
}
...
func TestToolsSurface(t *testing.T) {
	...
	for _, reg := range tools {
		if !strings.HasPrefix(reg.Name, "qsdev_windsurf_") {
			t.Errorf("tool %q lacks qsdev_windsurf_ prefix", reg.Name)
		}
...
func TestCapabilitiesTool(t *testing.T) {
	...
	if caps["tool_ceiling"].(int) != 100 {
		t.Errorf("tool_ceiling = %v, want 100", caps["tool_ceiling"])
	}
	...
	if caps["context_model"].(string) != "Cascade" {
		t.Errorf("context_model = %v, want Cascade", caps["context_model"])
	}
...
func TestConfigToolGated(t *testing.T) {
	...
	if payload["framework"] != string(aiframework.Windsurf) {
		t.Errorf("framework = %v, want windsurf", payload["framework"])
	}
...
func TestInfoTool(t *testing.T) {
	...
	if structured["framework"].(string) != string(aiframework.Windsurf) {
		t.Errorf("framework = %v, want windsurf", structured["framework"])
	}
