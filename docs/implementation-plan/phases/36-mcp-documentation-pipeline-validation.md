...
Phase 17 complete (test infrastructure framework — testscript E2E framework, custom commands like `json_path`, golden file infrastructure, CI pipeline). Phase 28 complete (MCP registry lifecycle management — registry format migration, `qsdev mcp list/enable/disable`, 40-tool ceiling, auto-detect logic, detect-and-offer logic). Phase 29 complete (local documentation pipeline — openzim-mcp, DevDocs MCP, man-mcp-server, `qsdev docs download/status/outdated/update/clean`, minisign content signing, prompt injection hardening pipeline).
...
- Documentation pipeline E2E test suite (openzim/DevDocs/man MCP servers, `qsdev docs` subcommands, skill routing)
...
6. Write `mcp-auto-detect-mysql-removal.txtar` — verify MySQL MCP auto-disabled when MySQL service removed:
   ...
   exec qsdev init --non-interactive --answers-file answers-with-mysql.yaml
   exec gdev mcp list --format json > list-before.json
   json_path list-before.json '.servers[]|select(.name=="mysql-mcp")' '.enabled' 'true'

   exec qsdev disable mysql
   exec gdev mcp list --format json > list-after.json
   ! json_path list-after.json '.servers[]|select(.name=="mysql-mcp")' '.enabled' 'true'
...
8. Write `mcp-yes-flag-no-bypass.txtar` — verify `--yes` does NOT auto-enable detect-and-offer servers:
   ...
   exec qsdev init --non-interactive --answers-file answers.yaml --yes
   exec gdev mcp list --format json > mcp-list.json
   ! json_path mcp-list.json '.servers[]|select(.name=="terraform-mcp")' '.enabled' 'true'
...
**Acceptance Criteria:**
- [ ] `qsdev mcp list --format json` shows all MVP servers with tool counts and security tiers
- [ ] `qsdev mcp enable` adds server to `.mcp.json`; resulting `.mcp.json` is valid JSON
- [ ] `qsdev mcp disable` removes server from `.mcp.json` cleanly; resulting `.mcp.json` is valid JSON
...
- [ ] `qsdev init --yes` does NOT auto-enable detect-and-offer servers

**Research Citations:**
- `phases/28-mcp-registry-lifecycle.md § Unit 28.1` — registry initialization, server metadata format, MVP server migration
- `phases/28-mcp-registry-lifecycle.md § Unit 28.2` — `qsdev mcp list/enable/disable` command implementation
...
- [ ] `qsdev mcp enable mcp-slack --yes` in non-interactive mode aborts with security confirmation required message
...
**Description:** Validate the `qsdev mcp check --compliance` command: `@yawlabs/mcp-compliance` suite runs against enabled servers, A-F grades are displayed per server, `--strict --min-grade B` exits non-zero when any server falls below B, and compliance checks block in the CI pipeline.
...
**Acceptance Criteria:**
- [ ] `qsdev mcp check --compliance --format json` produces per-server grade (A-F) and server name
...
- [ ] `qsdev mcp check --compliance --ci` integrates into CI pipeline and blocks on grade failures
...
**Description:** Validate the full local documentation pipeline from Phase 29: openzim-mcp starts and serves content when ZIM files are present, DevDocs MCP starts and serves when DevDocs JSON is present, man-mcp-server is always available on Linux, `qsdev docs download/status/outdated/update/clean` subcommands work correctly, and the `.claude/skills/lookup-docs/SKILL.md` routing file is generated with the correct priority order.

**Context:** Phase 29's documentation pipeline solves a key problem with AI-assisted development: Claude knows its training cutoff but not what's installed locally. By serving documentation as MCP tools from local files, the pipeline provides accurate, version-specific API references without network latency or privacy concerns. Graceful degradation is critical — the MCP servers must not crash when documentation files are absent; they must simply become unavailable. The `qsdev docs clean` command tests this path explicitly. The skill routing file is the mechanism through which Claude learns to prefer local docs over training data: incorrect priority ordering would cause Claude to use stale training data even when local docs are available.

**Desired Outcome:** A test suite verifying that each documentation MCP server starts when its data files are present and degrades gracefully when absent, that `qsdev docs` lifecycle subcommands work correctly, and that the generated skill routing file has the correct MCP server priority order.
...
5. Write `man-mcp-always-available.txtar` — verify man-mcp-server is always available on Linux:
   ...
   quick_mode: true
   ```
6. Write `gdev-docs-download.txtar` — verify `qsdev docs download` downloads for detected ecosystems:
   ```
   ...
   go 1.22
   ```
7. Write `gdev-docs-status.txtar` — verify `qsdev docs status` shows downloaded docs and disk usage:
   ```
   ...
   {"name": "Go"}
   ```
8. Write `gdev-docs-clean-degrades.txtar` — verify `qsdev docs clean` removes data and MCP servers degrade:
   ```
...
- [ ] `qsdev docs download --dry-run` shows download plan including detected ecosystem documentation
- [ ] `qsdev docs status --format json` reports downloaded doc count and disk usage
- [ ] `qsdev docs clean` removes downloaded docs; MCP servers that depended on them degrade gracefully
...
- `phases/29-local-documentation-pipeline.md § Unit 29.4` — `qsdev docs download/status/outdated/update/clean` lifecycle commands
...
**Description:** Validate the minisign-based content signing system from Phase 29: a complete sign/verify round-trip succeeds, tampering with signed content causes MCP startup failure, missing `.minisig` files cause MCP startup failure, and the `qsdev health` system reports signing failures. Verify that `qsdev docs update` displays a structural diff for user review when content changes.
...
**Desired Outcome:** A test suite verifying that the sign/verify round-trip functions correctly, that content tampering and missing signatures are caught at MCP startup with actionable error messages, that `qsdev health` surfaces signing failures as check failures, and that the update diff display shows meaningful structural differences.
...
4. Write `minisign-missing-signature.txtar` — verify missing signature causes failure with clear message:
   ...
   {"name": "Go"}
   ```
5. Write `gdev-health-signing-failure.txtar` — verify signing failures appear in `qsdev health` output:
   ```
   ...
   {"name": "Go"}
   ```
6. Write `docs-update-diff.txtar` — verify `qsdev docs update` shows structural diff for review:
   ```
...
- [ ] Signing failures appear in `qsdev health --format json` under a `doc-integrity` check
- [ ] `qsdev docs update --dry-run` shows structural diff when updated content differs from signed baseline
...
- [ ] MCP registry: `qsdev mcp list/enable/disable` produces valid `.mcp.json` throughout; 40-tool ceiling enforced
...
- [ ] Docs pipeline: `qsdev docs clean` removes data and triggers MCP graceful degradation
...
- [ ] Signing: signing failures surface in `qsdev health` as `doc-integrity` check failures
