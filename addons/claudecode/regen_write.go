...
// with a warning. A Skip (skip-if-exists) file that exists and is not
// unmodified qsdev output belongs to the user and is never replaced, not even
// with force.
func planRegenWrites(files []types.GeneratedFile, existing types.GeneratedState, projectRoot string, opts regenWriteOptions, warn io.Writer) (*regenPlan, error) {
	...
	owned := recordedOutput{projectRoot: projectRoot, claude: existing}
	...
	for _, f := range files {
		...
		if f.Strategy == types.Skip && exists && !owned.isOutput(f.Path, onDisk) {
			_, _ = fmt.Fprintf(warn, "Note: kept your existing %s (not generated by %s, or edited since); delete it to use the generated version\n",
				f.Path, branding.Get().AppName)
			plan.skipped++
			continue
		}
...
// recordedOutput answers whether on-disk content is unmodified qsdev output,
// as recorded in the Claude Code state or, lazily, in any of the project's
// state files (a top-level init records its files in the devinit state).
type recordedOutput struct {
	projectRoot string
	claude      types.GeneratedState
	project     []types.GeneratedState
	loaded      bool
}

func (r *recordedOutput) isOutput(relPath string, content []byte) bool {
	if state.IsRecordedOutput([]types.GeneratedState{r.claude}, relPath, content) {
		return true
	}
	if !r.loaded {
		r.loaded = true
		// A state file that fails to load only means fewer files count as
		// qsdev output, so more are kept: the safe direction.
		r.project, _ = state.LoadProjectStates(r.projectRoot)
	}
	return state.IsRecordedOutput(r.project, relPath, content)
}
