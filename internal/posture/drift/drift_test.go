...
func TestDetect_Aggregation(t *testing.T) {
	dir := t.TempDir()
	...
	report := Detect(dir, genState, enabledTools)
...
func TestDetect_EmptyState(t *testing.T) {
	dir := t.TempDir()

	// No git repo, no files.
	report := Detect(dir, types.GeneratedState{}, nil)
...
func BenchmarkDetect(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		Detect(dir, genState, enabledTools)
	}
}
