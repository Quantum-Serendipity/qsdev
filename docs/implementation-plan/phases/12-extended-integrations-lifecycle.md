...
- `gdev enable <tool>` / `gdev disable <tool>` commands for granular tool management
- `gdev status` showing all tools with enabled/disabled state and generated files
- `gdev list` showing available tools, modules, skills, and plugins with descriptions
...
**Description:** Implement the core tool lifecycle infrastructure: a tool registry that tracks what's available and enabled, file ownership that maps tools to their generated artifacts, and `gdev enable`/`gdev disable`/`gdev status`/`gdev list` commands that cleanly add and remove tools with surgical shared-file editing.

**Context:** The current architecture only supports `gdev init` (generate everything at once) and `gdev init --update` (regenerate). There is no way to toggle individual tools after initial setup. A developer who tries Version-Sentinel and decides they don't want it has to manually hunt down and delete its files, remove its entries from settings.json and CLAUDE.md, and clean up devenv.nix packages. This friction discourages experimentation and makes tool adoption a one-way door.

The lifecycle system turns tool adoption into a reversible, low-risk decision. The saved answers file (`.devinit/.gdev-init-answers.yaml`) already records choices; the state file (`.devinit/.gdev-init-state.yaml`) already records generated files with hashes. What's missing is: (a) file ownership linking files to the tool that created them, (b) shared-file surgery to remove a tool's contributions from files that multiple tools write to, and (c) commands that orchestrate the enable/disable flow.

**Desired Outcome:** `gdev enable semgrep` adds Semgrep to the project. `gdev disable semgrep` cleanly removes it — deleting `.semgrep.yml`, removing `semgrep` from devenv.nix packages, removing the Semgrep pre-commit hook, and removing the Semgrep CI step from generated workflows. No manual cleanup needed.
...
8. Implement `gdev status`:
   ...
   - Support `gdev status --json` for machine-readable output.
9. Implement `gdev list`:
   ...
   - Support `gdev list --category security` filtering.
...
**Acceptance Criteria:**
- [ ] `gdev enable semgrep` generates `.semgrep.yml`, adds package to devenv.nix, adds hook to pre-commit config, adds CI step
- [ ] `gdev disable semgrep` removes `.semgrep.yml`, removes package from devenv.nix, removes hook, removes CI step
- [ ] `gdev disable version-sentinel` removes plugin install instructions, cleans CLAUDE.md section, removes hooks from settings.json
- [ ] `gdev disable semble` removes MCP entry from .mcp.json, removes `.claude/agents/semble-search.md`
- [ ] `gdev status` shows all tools with correct enabled/disabled state
- [ ] `gdev list` shows all available tools grouped by category
...
- [ ] `gdev enable <tool>` fails cleanly if prerequisites are missing
- [ ] `gdev enable <tool>` fails cleanly if conflicting tool is enabled
...
**Desired Outcome:** `gdev init` generates Semgrep config for detected ecosystems. `gdev disable semgrep` cleanly removes it. `gdev enable semgrep` adds it back.
...
- [ ] `gdev enable semgrep` / `gdev disable semgrep` work cleanly
...
- [ ] `gdev enable gitleaks` / `gdev disable gitleaks` work cleanly
...
**Desired Outcome:** Docker projects get a scan-sign-verify pipeline in CI and a local `gdev docker scan` command, cleanly removable via `gdev disable container-security`.
...
6. Add `gdev docker scan <image>` command running Syft+Grype locally.
...
- [ ] `gdev disable container-security` removes all three tools' artifacts
...
- [ ] Local `gdev docker scan` works without CI infrastructure
...
**Desired Outcome:** Weekly CI license scans enforce license policy. `gdev disable license-compliance` removes it entirely.
...
5. Add `gdev license check` command for local scanning.
...
- [ ] `gdev enable license-compliance` / `gdev disable license-compliance` work cleanly
...
**Desired Outcome:** `gdev init` with services detected generates SecretSpec config. `gdev disable secretspec` removes it cleanly.
...
- [ ] `gdev enable secretspec` / `gdev disable secretspec` work cleanly
...
**Context:** Unlike other tools that contribute sections to shared files, CI workflows are fully regenerated from the current enabled-tool set on every `gdev enable`/`gdev disable` call. This is the one shared file type where surgical editing is too complex — workflow YAML has dependency ordering, matrix strategies, and conditional jobs that interact non-locally. Full regeneration from the tool registry is cleaner and more reliable.

**Desired Outcome:** CI workflows always reflect the exact set of currently enabled tools. `gdev enable semgrep` regenerates the workflow to include the Semgrep step. `gdev disable semgrep` regenerates without it.
...
**Desired Outcome:** Default `.mcp.json` has 3-4 servers. Each is individually toggleable via `gdev enable`/`gdev disable`.
...
6. Add `gdev list --category ai-agent` to show all available MCP servers.

**Acceptance Criteria:**
- [ ] Context7 configured by default
- [ ] `gdev disable context7` removes it from .mcp.json
- [ ] `gdev enable postgres-mcp` adds it to .mcp.json
...
**Desired Outcome:** Pre-commit hooks stay under 10 seconds total. `gdev disable gitleaks` also removes its pre-commit hook.
...
**Acceptance Criteria:**
- [ ] Gitleaks hook added/removed with `gdev enable/disable gitleaks`
- [ ] Semgrep hook added/removed with `gdev enable/disable semgrep`
- [ ] Commitlint independently toggleable via `gdev enable/disable commitlint`
...
**Description:** Register git-cliff as a lifecycle-managed tool generating `cliff.toml` configuration and a `gdev changelog` command.
...
**Desired Outcome:** `gdev enable changelog` adds git-cliff config and the `gdev changelog` command. `gdev disable changelog` removes it.
...
4. Add `gdev changelog` command (thin wrapper around `git-cliff`).
...
- [ ] `gdev enable changelog` / `gdev disable changelog` work cleanly
...
**Context:** Phase 4 deployed attach-guard and Trail of Bits skills. Phase 11 added agent-postmortem, Version-Sentinel, and semble. These were originally generated as part of `gdev init` with no individual toggle. This unit retrofits them into the lifecycle system by defining their `Tool` registry entries and `FileOwnership` mappings, enabling `gdev disable version-sentinel` to work even though the tool was deployed by Phase 4/11 code.

**Desired Outcome:** All previously deployed tools are individually toggleable via `gdev enable`/`gdev disable`.
...
**Acceptance Criteria:**
- [ ] `gdev disable version-sentinel` cleanly removes all VS artifacts
- [ ] `gdev disable semble` removes MCP entry and/or agent file
- [ ] `gdev disable agent-postmortem` removes skill file
- [ ] `gdev enable <tool>` re-adds previously disabled tools
- [ ] Existing projects can adopt lifecycle management without re-running `gdev init`
...
- [ ] `gdev enable semgrep` → verify files created → `gdev disable semgrep` → verify files cleaned up (no orphaned artifacts)
- [ ] `gdev status` accurately reflects all enabled/disabled tools
- [ ] `gdev list` shows all 15+ available tools with correct categories and descriptions
...
- [ ] `gdev init --yes` on a fresh project generates correct default tool set
- [ ] `gdev init --update` respects current enable/disable state
