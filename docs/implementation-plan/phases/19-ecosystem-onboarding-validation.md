...
Validate that `qsdev init` correctly detects, configures, and generates a working security-hardened development environment for every supported language ecosystem — both for new (greenfield) projects and existing (brownfield) projects. At the end of this phase, every Tier 1-2 ecosystem has verified new-project and existing-project test cases, with polyglot combinations tested.

## Dependencies

Phase 17 complete (test infrastructure). Phases 1-8 complete (all ecosystem modules, generation pipeline). Phase 6 complete (wizard, answers file). Phase 13 complete (project configuration — `.qsdev.yaml` and Join mode for team standards).

## Phase Outputs

- Verified greenfield `qsdev init` for all 27 ecosystems
- Verified brownfield `qsdev init` against real open-source repos (Tier 1-2)
...
**Description:** For each Tier 1 ecosystem, create a fresh project from scratch, run `qsdev init --answers-file`, and verify all generated output matches the ecosystem's verification checklist. Each package manager variant is tested independently.

**Context:** Greenfield validation is the simplest path — no existing configs to merge, no detection ambiguity from co-located ecosystems. This is the baseline: if `qsdev init` cannot produce correct output for a clean new project, nothing else works. The research artifact identifies exact project creation commands and verification checklists for every ecosystem, including per-package-manager variants within JS/TS (4 PMs) and Python (3 PMs). Each test uses `--answers-file` to run non-interactively with known-good wizard responses, producing deterministic output that can be asserted against.
...
6. Python — pip variant:
   - Create `requirements.txt` then `qsdev init`.
...
11. Java (Maven):
    - `mvn archetype:generate` (non-interactive) then `qsdev init`.
...
12. Kotlin (Gradle):
    - `gradle init --type kotlin-application --dsl kotlin` then `qsdev init`.
...
13. .NET:
    - `dotnet new console` then `qsdev init`.
...
14. Docker:
    - Create a `Dockerfile` then `qsdev init`.
...
15. Terraform:
    - Create `main.tf` with provider block then `qsdev init`.
...
**Description:** Clone real open-source repos and run `qsdev init` to test detection accuracy, existing config handling, and merge behavior against production codebases.
...
17. Test Join mode (existing `.qsdev.yaml`): create a brownfield fixture with a pre-existing `.qsdev.yaml` team configuration file. Run `qsdev init --yes`. Verify that Join mode is detected (existing `.qsdev.yaml` drives configuration instead of the wizard), team standards from `.qsdev.yaml` are applied, and the onboarding experience respects the committed project configuration.

**Acceptance Criteria:**
- [ ] All 14 brownfield repos onboard without errors or crashes
- [ ] Join mode correctly detected and applied when `.qsdev.yaml` is present
...
**Description:** Create new projects and verify `qsdev init` for all Tier 2-4 ecosystems. Tier 2 gets full security config testing. Tier 3-4 verify detection and basic devenv.nix generation only.
...
1. Tier 2 — PHP (Composer):
   - `composer init --no-interaction` then `qsdev init`.
...
3. Tier 2 — Scala (sbt):
   - `sbt new scala/scala3.g8` then `qsdev init`.
...
4. Tier 2 — Helm:
   - `helm create test-helm` then `qsdev init`.
...
5. Tier 2 — Ansible (Galaxy):
   - Create `playbook.yml` and `requirements.yml` then `qsdev init`.
...
6. Tier 2 — Bash/Shell:
   - Create `script.sh` with bash shebang then `qsdev init`.
...
7. Tier 2 — C/C++ (CMake):
   - Create `CMakeLists.txt` and `src/main.cpp` then `qsdev init`.
...
8. Tier 3 — Elixir (Mix):
   - `mix new test_elixir` then `qsdev init`.
...
11. Tier 3 — Haskell (Cabal):
    - `cabal init --minimal --non-interactive` then `qsdev init`.
...
12. Tier 3 — Clojure (deps.edn):
    - Create `deps.edn` then `qsdev init`.
...
**Description:** Test multi-ecosystem detection and composition by running `qsdev init` against real polyglot repositories that combine two or more language ecosystems.
...
5. Lock file presence/absence:
   ...
   - Verify warnings are informational, not errors — `qsdev init` completes successfully.
6. Nested projects:
...
8. Empty project:
   - Completely empty directory — verify `qsdev init` handles gracefully.
   ...
   - Verify `qsdev init --yes` on empty project produces minimal valid output (base devenv.nix with no ecosystem modules).
