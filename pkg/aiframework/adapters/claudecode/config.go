...
func (a *Adapter) renderSettings(answers types.WizardAnswers, cfg ccaddon.Config, input *aiframework.PolicyInput) (*types.GeneratedFile, error) {
...
func addRemoteMCPServers(mcp *types.GeneratedFile, remote []aiframework.MCPServerSpec) (*types.GeneratedFile, error) {
	...
	if doc.MCPServers == nil {
		doc.MCPServers = make(map[string]ccaddon.MCPServerEntry, len(remote))
	}
	for _, s := range remote {
		...
		doc.MCPServers[s.Name] = ccaddon.MCPServerEntry{Type: entryType, URL: s.URL, Env: s.Env}
	}
