...
func buildAllRules() []sarifRule {
	return []sarifRule{
		...
		{ID: ruleID("state-corrupt"), ShortDescription: sarifMultiText{Text: "A generation state file failed to load, so the assessment lacks its inputs"}, DefaultConfig: sarifConfig{Level: "error"}},
		{ID: ruleID("version-drift"), ShortDescription: sarifMultiText{Text: "Configuration was generated by a different version"}, DefaultConfig: sarifConfig{Level: "note"}},
	}
}
...
const (
	...
	categoryVersionDrift     = "Version Drift"
)
...
func driftToSARIF(categoryName string, f drift.Finding) (id, level string) {
	...
	case categoryMarkerIntegrity:
		// Keep the finding's own severity: a missing CLAUDE.md is an error,
		// not a broken-marker warning.
		return ruleID("markers-broken"), severityToLevel(f.Severity)
	...
	case posture.StateFilesCategory:
		// A state file that fails to load strips the defense and conformance
		// checks of their inputs, so it is always an error.
		return ruleID("state-corrupt"), "error"
	case categoryVersionDrift:
		return ruleID("version-drift"), "note"
	default:
		return "", "" // Unknown categories are skipped
	}
}

// severityToLevel maps a drift severity to its SARIF result level.
func severityToLevel(sev drift.Severity) string {
	switch sev {
