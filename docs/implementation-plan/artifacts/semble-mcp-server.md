# Semble - MCP Server Implementation

- **Source**: https://raw.githubusercontent.com/MinishLab/semble/main/src/semble/mcp.py
        ...
        if cache_key in self._tasks:
            ...
            if _is_git_url(source):
                self._tasks[cache_key] = asyncio.create_task(
                    asyncio.to_thread(
                        SembleIndex.from_git, source, ref=ref, model=self._model,
                ...
                self._tasks[cache_key] = asyncio.create_task(
                    asyncio.to_thread(
                        SembleIndex.from_path, cache_key, model=self._model,
                        include_text_files=self._include_text_files,
                    )
                )
