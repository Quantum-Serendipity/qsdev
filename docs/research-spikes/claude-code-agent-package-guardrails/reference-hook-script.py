...
def check_npm_age(package_name: str) -> Optional[float]:
    ...
    """
    # npm registry JSON: GET /<package> returns all version metadata.
    # We only need the "time" field which maps version -> ISO date.
    ...
    # "modified" is the most recent publication event.
    modified = time_map.get("modified")
    if not modified:
        return None

    pub_date = datetime.fromisoformat(modified.replace("Z", "+00:00"))
