...
// at path, stamping each package with the OSV ecosystem identifier eco. The
// ecosystem is supplied by the caller (derived from pkg/ecosystem) rather than
// inferred inside the parser, so a single parser can serve several ecosystems.
type lockParser func(path, eco string) ([]osvPackage, error)
...
// once a parser is registered here — the scanner never silently drifts from the
// catalog.
...
// before loose manifests like requirements.txt. This preserves the original
// detection priority — authoritative lock files win over partial manifests —
// without hardcoding individual file names. The input slice is never mutated.
...
// without it.
func parseGoSum(path, eco string) ([]osvPackage, error) {
	...
	for sc.Scan() {
		...
		seen[key] = true
		pkgs = append(pkgs, osvPackage{Name: mod, Version: ver, Ecosystem: eco})
	}
	return pkgs, sc.Err()
}
...
func parseNPMLock(path, eco string) ([]osvPackage, error) {
...
// file (Cargo.lock, poetry.lock, uv.lock) using the BurntSushi/toml parser, so
// quoting, escapes, inline tables, and array values are handled correctly rather
// than by line scanning. Duplicate (name, version) coordinates are collapsed;
// distinct versions of the same package are preserved.
func parseTOMLPackages(path, eco string) ([]osvPackage, error) {
	var lock tomlLock
	...
	for _, p := range lock.Package {
		...
		seen[key] = true
		pkgs = append(pkgs, osvPackage{Name: p.Name, Version: p.Version, Ecosystem: eco})
	}
	return pkgs, nil
}
...
func parsePipfileLock(path, eco string) ([]osvPackage, error) {
	data, err := os.ReadFile(path) //nolint:gosec // project-root lock file
	if err != nil {
		return nil, err
	}
...
func parseRequirementsTxt(path, eco string) ([]osvPackage, error) {
