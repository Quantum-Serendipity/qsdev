...
This document describes every file generated by qsdev, its purpose, merge strategy during updates, and key contents.

## Merge Strategies

Each generated file has an assigned merge strategy that controls how it is handled during `qsdev init --update`:
...
| | |
|---|---|
| **Generated by** | `qsdev devenv init` / `qsdev init` |
...
| | |
|---|---|
| **Generated by** | `qsdev devenv init` / `qsdev init` (when `--direnv` is enabled) |
| **Merge strategy** | `overwrite` |
...
| | |
|---|---|
| **Generated by** | `qsdev claude init` / `qsdev init` |
...
| | |
|---|---|
| **Generated by** | `qsdev claude init` / `qsdev init` (when `safety-block` hook is enabled) |
| **Merge strategy** | `overwrite` |
...
| | |
|---|---|
| **Generated by** | `qsdev claude init` / `qsdev init` / `qsdev claude add-skill` |
...
As library-managed files, these are always updated to the latest version during `qsdev init --update`.
...
Language-specific coding conventions and security policies. Updated automatically on `qsdev init --update`.

### `.mcp.json`

| | |
|---|---|
| **Generated by** | `qsdev claude init` / `qsdev init` (when MCP servers are configured) |
| **Merge strategy** | `three-way-merge` |
...
| | |
|---|---|
| **Generated by** | `qsdev init --nix-hardening-guide` / `qsdev devenv init --nix-hardening-guide` |
| **Merge strategy** | `overwrite` |
...
Saved wizard answers used by `qsdev init --update` to regenerate files without re-running the wizard. Contains selected languages, services, permission level, skills, hooks, MCP servers, and infrastructure profile.

### `.devinit/.qsdev-init-state.yaml`

Tracks every generated file with its path, content hash, and base content (for three-way merge). Used to detect user modifications during `qsdev init --update`.

### `.devenv/.gdev-state.yaml`

Per-addon state for `qsdev devenv update`.

### `.claude/.gdev-claude-state.yaml`

Per-addon state for `qsdev claude update`. Also stores template and skill library version identifiers.
