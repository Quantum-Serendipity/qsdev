...
// int-backed enumerations whose values are the contiguous range 0..n-1, so
// each enum type declares its names once instead of hand-copying the logic.
package enumtext

import "fmt"

// Enum maps the values of an int-backed enumeration to their text names:
// Names[i] is the text form of value i. Validity is decided by index range,
// never by comparing against the invalid sentinel, so a legitimate value may
// be named "unknown".
...
func (e Enum[T]) Valid(v T) bool {
	return int(v) >= 0 && int(v) < len(e.names)
}

// String returns the name of v, or the invalid sentinel when v is out of range.
...
// MarshalText returns the name of v, or an error when v is out of range.
